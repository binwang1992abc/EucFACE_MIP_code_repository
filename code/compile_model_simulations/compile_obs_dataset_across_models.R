compile_obs_dataset_across_models <- function(p.mod.list, 
                                              n.mod.list) {
    ...
    ### source directory
    source.dir <- paste0("D:/Analysis/EucFACE_MIP/compile_output")
    ...
    ##################################################################
    #### loop through models for observed period, 
    #### to create rds files 
    ...
    for (i in co2.levels) {
        for (j in clim.levels) {
            ...
            for (mod.abb in p.mod.list) {
                ...
                if (mod.abb=="CABLP") {
                    modDF <- read.csv(paste0(source.dir, "/", mod.abb, 
                                             "/forest/EUC_", mod.abb, "_OBS_", j, "_", i, "_NOP_D.csv"))
                    ...
                    modDF <- read.csv(paste0(source.dir, "/", mod.abb, "/EUC_", 
                                             mod.abb, "_OBS_", j, "_", i, "_NOP_D.csv"))
                } else {
                    modDF <- read.csv(paste0(source.dir, "/", mod.abb, 
                                             "/EUC_", mod.abb, "_OBS_", j, "_", i, "_NOP_D.csv"))
                }
                ...
                ## revised by B.W.
                modDF$Date <- as.Date(paste0(modDF$YEAR, '-01-01'))+modDF$DOY-1
    ...
    #    ambDF <- readRDS(paste0(out.dir, "/MIP_OBS_", i, "_AMB_daily.rds"))
    #    eleDF <- readRDS(paste0(out.dir, "/MIP_OBS_", i, "_ELE_daily.rds"))
    #    
    ...
    #    
    #    ### save output
    #    saveRDS(annDF1, paste0(out.dir, "/MIP_OBS_", i, "_AMB_annual.rds"))
    #    saveRDS(annDF2, paste0(out.dir, "/MIP_OBS_", i, "_ELE_annual.rds"))
    #    
    #    
    #}
